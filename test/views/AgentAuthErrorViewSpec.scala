...
class AgentAuthErrorPageViewSpec extends AnyWordSpec with Matchers with GuiceOneAppPerSuite {
  ...
  lazy val pText = "You cannot view this client's information." +
    " Your client needs to authorise you as their agent before you can sign into this service." +
  ...
  "AgentAuthErrorPageView when there is an error" should {
    lazy val view  = agentAuthErrorPageView("Authentication Error", "You're not enrolled for the agent service")
    lazy implicit val document: Document = Jsoup.parse(view.body)

    "have a page heading of" in {
      elementText(Selectors.pageHeading) shouldBe "You're not enrolled for the agent service"
    }

    "have text of " in {
      elementText(Selectors.p1) shouldBe pText
    }
  }
}